{ config, lib, pkgs, ... }:

let
  cfg = config.ahiru.primaryUser;
in
{
  # Define options for primary user (so other modules can reference it)
  options.ahiru.primaryUser = {
    name = lib.mkOption {
      type = lib.types.str;
      default = "dan";
      description = "Primary user name";
    };
    uid = lib.mkOption {
      type = lib.types.int;
      default = 1000;
      description = "Primary user UID";
    };
    email = lib.mkOption {
      type = lib.types.str;
      default = "me@ahiru.pl";
      description = "Primary user email";
    };
    authorizedKeys = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIN/7ZMH3Q2QQQFD6Ugtv8Lii2WTdYV3GM0aYa5Bu+Bvw me@ahiru.pl"
        "ssh-rsa  tojad99@gmail.com"
      ];
      description = "SSH authorized keys for primary user";
    };
  };

  config = {
    # Generate SSH key for primary user on first boot
    system.activationScripts.primaryUserSshKey = ''
      keyfile="/home/${cfg.name}/.ssh/id_ed25519"
      sshdir="/home/${cfg.name}/.ssh"
      marker="/var/lib/first-boot-ssh-key-shown"

      if [[ ! -f "$keyfile" ]]; then
        mkdir -p "$sshdir"
        ${pkgs.openssh}/bin/ssh-keygen -t ed25519 -f "$keyfile" -N "" -C "${cfg.name}@ahiru"
        chown -R ${toString cfg.uid}:100 "$sshdir"
        chmod 700 "$sshdir"
        chmod 600 "$keyfile"
        chmod 644 "$keyfile.pub"
      fi

      # Show the key once after first generation (for adding to GitHub)
      if [[ -f "$keyfile.pub" && ! -f "$marker" ]]; then
        echo ""
        echo "=============================================="
        echo "  SSH KEY GENERATED - ADD TO GITHUB"
        echo "=============================================="
        echo ""
        cat "$keyfile.pub"
        echo ""
        echo "Add this key at: https://github.com/settings/ssh/new"
        echo ""
        echo "Then clone the config repo:"
        echo "  git clone git@github.com:mruwnik/media-server.git ~/nixos-config"
        echo ""
        echo "=============================================="
        touch "$marker"
      fi
    '';

    # Main user
    users.users.${cfg.name} = {
      isNormalUser = true;
      uid = cfg.uid;
      extraGroups = [ "wheel" "torrents" ];
      openssh.authorizedKeys.keys = cfg.authorizedKeys;
    };

    # Additional users for Samba access
    users.users.nadia = {
      isNormalUser = true;
      uid = 1002;
      description = "Nadia";
    };

    users.users.rumun = {
      isNormalUser = true;
      uid = 1003;
      description = "Rumun";
    };

    # Passwordless sudo for wheel
    security.sudo.wheelNeedsPassword = false;

    # Service users will be added by their respective modules:
    # - calibre (calibre-web)
    # - torrents (rtorrent)
    # - radicale
    # - forgejo creates its own
    # - cryptpad creates its own
  };
}
